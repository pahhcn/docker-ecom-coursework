...
# 注意：不直接挂载 ~/.minikube，而是在容器内复制证书，避免权限问题
docker run -d \
  --name jenkins-local \
  --network host \
...
# 配置 Minikube 证书和配置（如果 minikube 正在运行）
if minikube status &> /dev/null; then
    echo "🔐 配置 Minikube 证书和配置..."
    ...
    docker exec -u root jenkins-local bash -c "
        ...
        mkdir -p /root/.minikube/machines/minikube
    " 2>/dev/null || true
    
    # 复制证书文件到容器（使用 docker cp 避免挂载导致的权限问题）
    if [ -f ~/.minikube/ca.crt ]; then
        docker cp ~/.minikube/ca.crt jenkins-local:/var/jenkins_home/.minikube/ca.crt 2>/dev/null || true
        docker cp ~/.minikube/ca.crt jenkins-local:/root/.minikube/ca.crt 2>/dev/null || true
        ...
        docker cp ~/.minikube/profiles/minikube/client.crt jenkins-local:/root/.minikube/profiles/minikube/client.crt 2>/dev/null || true
        ...
        docker cp ~/.minikube/profiles/minikube/client.key jenkins-local:/root/.minikube/profiles/minikube/client.key 2>/dev/null || true
        
        # 复制 minikube 配置文件（让 minikube 命令能找到集群）
        docker cp ~/.minikube/profiles/minikube/config.json jenkins-local:/root/.minikube/profiles/minikube/config.json 2>/dev/null || true
        docker cp ~/.minikube/machines/minikube/config.json jenkins-local:/root/.minikube/machines/minikube/config.json 2>/dev/null || true
        docker cp ~/.minikube/machines/minikube/id_rsa jenkins-local:/root/.minikube/machines/minikube/id_rsa 2>/dev/null || true
        docker cp ~/.minikube/machines/minikube/id_rsa.pub jenkins-local:/root/.minikube/machines/minikube/id_rsa.pub 2>/dev/null || true
        
        # 修复 machines/config.json 中的路径（从宿主机路径改为容器路径）
        docker exec -u root jenkins-local sed -i "s|$HOME/.minikube|/root/.minikube|g" /root/.minikube/machines/minikube/config.json 2>/dev/null || true
        
        # 生成 kubeconfig for jenkins user
        docker exec -u root jenkins-local bash -c "
...
EOF
            # 同时为 root 用户生成 kubeconfig
            cat > /root/.kube/config << 'EOFROOT'
apiVersion: v1
clusters:
- cluster:
